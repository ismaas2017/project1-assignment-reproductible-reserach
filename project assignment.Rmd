...
data1 %>%select(steps, date) %>% group_by(date) %>%summarise(total=sum(steps,na.rm=TRUE))%>%with(hist(total, main="total PM2.5 emission from all sources", xlab="Year", ylab="Total Emissions"))
```